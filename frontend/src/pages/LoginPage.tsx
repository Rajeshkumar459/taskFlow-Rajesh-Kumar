...
const FEATURES = [
  { icon: '✦', text: 'Kanban boards with drag-and-drop' },
  { icon: '✦', text: 'Real-time collaboration via live updates' },
  { icon: '✦', text: 'Priority-sorted task management' },
  { icon: '✦', text: 'Team roles and permissions' },
]
    ...
    <Box sx={{ display: 'flex', minHeight: '100vh' }}>
      ...
      >
        ...
        <Typography variant="body1" sx={{ color: 'rgba(255,255,255,0.7)', mb: 5, lineHeight: 1.7 }}>
          A clean, real-time project tracker built for modern teams.
        </Typography>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          {FEATURES.map((f) => (
            <Box key={f.text} sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
              <Typography sx={{ color: 'rgba(255,255,255,0.45)', fontSize: '0.65rem' }}>{f.icon}</Typography>
              <Typography variant="body2" sx={{ color: 'rgba(255,255,255,0.82)', fontWeight: 500 }}>{f.text}</Typography>
            </Box>
          ))}
        </Box>
      </Box>
